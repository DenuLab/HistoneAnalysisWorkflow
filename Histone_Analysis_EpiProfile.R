...
labels <- read.csv("histone_ratios_basic.csv", header = FALSE)
...
labels <- unlist(sapply(labels, function(x){unlist(str_sub(x, -5, -1))}))
...
data <- read.csv("histone_ratios_multi.csv", skip = 1)
...
# Now add your sample names
# This script will pull the file names from the EpiProfiler Results
labels <- read.csv("histone_ratios_multi.csv", header = FALSE)
labels <- labels[1,2:ncol(data)]
# Now you'll chose a subset of these labels to use as your sample IDs. The command below uses the last parts of each word to create a label
# You can choose how long you want this label to be by changing the first number (-5).
# -5 keeps the last 5 letters, -11 would keep the last 11 letters, etc. 
...
write.xlsx(final, file = "filename.xlsx", sheetName = "Final")
write.xlsx(final_deconv, file = "filename.xlsx", sheetName = "Final_deconv", append = TRUE)
write.xlsx(normalized, file = "filename.xlsx", sheetName = "Normalized", append = TRUE)
